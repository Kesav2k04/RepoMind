...
const AGENTS: Array<{ role: AgentRole; label: string; description: string }> = [
  { role: 'architecture', label: 'Architecture', description: 'Maps entry points and boundaries.' },
  { role: 'risk', label: 'Risk', description: 'Surfaces fragile and exposed areas.' },
  { role: 'testing', label: 'Testing', description: 'Finds verification gaps.' },
  { role: 'history', label: 'History', description: 'Traces churn and change context.' },
]
...
function Pipeline({ job }: { job?: AnalysisJob }) {
  ...
  return <section className={`pipeline ${isActive ? 'pipeline--active' : ''}`} aria-label="RepoMind orchestration pipeline">
    ...
    <div className="pipeline-specialists">
      <span className="pipeline-specialists__label">{specialistLabel}</span>
    ...
    <span className="pipeline-arrow" aria-hidden="true">→</span>
    <PipelineStage label={masterLabel} detail={complete ? `${decisions || 'Evidence'} decisions recorded` : failed ? 'Stopped safely; no unsupported synthesis' : masterEvent?.action ?? masterEvent?.message ?? (completedRoles ? `Comparing ${completedRoles}/4 specialist reports` : 'Compares and resolves evidence')} state={masterState} />
    <span className="pipeline-arrow" aria-hidden="true">→</span>
...
function NativePriorities({ job }: { job: AnalysisJob }) {
  ...
  if (!priorities.length) return <aside className="native-priorities native-priorities--empty" aria-label="GPT-5.6 evidence-bound priorities">
    <div><p className="eyebrow eyebrow--accent">GPT-5.6 root synthesis</p><h3>No additional priorities nominated</h3><p>GPT-5.6 source specialists produced only firewall-verified claims, and the root returned no extra presentation priorities. The verified handoff remains available.</p></div>
  </aside>
...
function MapBranch({ node, depth, onSelect, selectedPath }: { node: RepositoryMapNode; depth: number; onSelect: (node: RepositoryMapNode) => void; selectedPath?: string }) {
  ...
  return <li className="map-tree__item">
    <div className={`map-tree__row ${selectedPath === node.path ? 'map-tree__row--selected' : ''}`} style={{ paddingLeft: `${depth * 1.05 + .45}rem` }}>
      ...
      <span className={severityClass(node.risk)}>{node.risk === 'info' ? 'informational' : node.risk}</span>
    </div>
...
function RepoMindApp() {
  ...
  async function startAnalysis(candidateUrl: string, candidateTask = taskDescription) {
    ...
    try { setJob(await createAnalysis(normalizedUrl, candidateTask)) }
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    void startAnalysis(repoUrl, taskDescription)
  }
  ...
  function retryAnalysis() {
    ...
    if (!retryUrl) return
    setRepoUrl(retryUrl)
    const retryTask = job?.taskDescription ?? taskDescription
    setTaskDescription(retryTask)
    void startAnalysis(retryUrl, retryTask)
  }
